...
class residual_block(nn.Module):
    def __init__(self, channel, stride=1, mult=4, kernel=3):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReflectionPad2d(pad),
            nn.Conv2d(
                channel // mult, channel, kernel_size=kernel, padding=0, bias=True
            ),
        )
        self.init_layers()
    ...
    def __init__(self, in_ch, out_ch, sp_steps=2, n_blocks=2, kernel=3):
        ...
        for i in range(n_blocks):
            self.block_list.append(
                residual_block(out_ch * 4**sp_steps, stride=1, mult=4, kernel=kernel)
            )
...
class RevResNet(nn.Module):
    def __init__(
        ...
        print("")
        print(
            " == Building Reversible Residual Network %d Blocks== "
            % (sum(nBlocks) * 3 + 1)
        )
        if not nChannels:
        ...
        self.stack = self.block_stack(
            residual_block, nChannels, nBlocks, nStrides, mult=mult, kernel=kernel
        )

        self.channel_reduction = channel_reduction(
            nChannels[-1], hidden_dim, sp_steps=sp_steps, kernel=kernel
        )
